...
public class OracleRDFNoSQLDatabase extends DatabaseInterface {
    ...
    public ArrayList<PyObject> OracleNoSQLRunSPARQL(String sparql)
    {
        ...
        Reasoner res = ReasonerRegistry.getRDFSReasoner();
        InfModel inf = ModelFactory.createInfModel(res, default_model);
        Dataset ds = new DatasetImpl(inf);
        
        while(it.hasNext()){
            Node n = it.next();
            Graph g = datasetGraph.getGraph(n);
            Model base_model = ModelFactory.createModelForGraph(g);
            Reasoner reasoner = ReasonerRegistry.getRDFSReasoner();
            InfModel inf_model = ModelFactory.createInfModel(reasoner, base_model);
            ds.addNamedModel(n.getURI(),inf_model);
        }
